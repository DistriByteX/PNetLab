...
# Modified: all PNETLab packages are pulled from a single zip hosted on GitHub
# instead of individually from labhub.eu.org
# CONSTANTS
GREEN='\033[32m'
RED='\033[31m'
NO_COLOR='\033[0m'
...
echo -e "${GREEN}Downloading PNETLAB PACKAGES from GitHub ...${NO_COLOR}"
rm -rf /tmp/* &>/dev/null
cd /tmp 2>&1 >/dev/null

# --- Single download + extract of your GitHub-hosted package bundle ---
wget --content-disposition -q --show-progress -O "/tmp/$PACKAGES_ZIP" "$URL_PACKAGES"
if [ $? -ne 0 ] || [ ! -s "/tmp/$PACKAGES_ZIP" ]; then
    echo -e "${RED}Failed to download package bundle from $URL_PACKAGES${NO_COLOR}"
    exit 1
fi

mkdir -p "$PACKAGES_DIR"
unzip -o "/tmp/$PACKAGES_ZIP" -d "$PACKAGES_DIR" &>/dev/null

echo -e "${GREEN}Installing packages${NO_COLOR}"

# Kernel (nested zip inside the bundle)
dpkg-query -l | grep linux-image-5.17.15-pnetlab-uksm-2 | grep 5.17.15-pnetlab-uksm-2-1 -q
if [ $? -ne 0 ]; then
    unzip -o "$PACKAGES_DIR/$KERNEL" -d /tmp &>/dev/null
    dpkg -i /tmp/pnetlab_kernel/*.deb
fi

# docker-ce (nested zip inside the bundle)
dpkg-query -l | grep docker-ce -q
if [ $? -ne 0 ]; then
    unzip -o "$PACKAGES_DIR/pre-docker.zip" -d /tmp &>/dev/null
    dpkg -i /tmp/pre-docker/*.deb
fi

# swtpm (nested zip inside the bundle)
dpkg-query -l | grep swtpm -q
if [ $? -ne 0 ]; then
    unzip -o "$PACKAGES_DIR/swtpm-focal.zip" -d /tmp &>/dev/null
    dpkg -i /tmp/swtpm-focal/*.deb
fi

# Individual .deb packages already sitting in the extracted bundle
dpkg-query -l | grep pnetlab-docker | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-docker_*.deb
fi

dpkg-query -l | grep pnetlab-schema | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-schema_*.deb
fi

dpkg-query -l | grep pnetlab-guacamole | grep 6.0.0-7 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-guacamole_*.deb
fi

dpkg-query -l | grep pnetlab-vpcs | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-vpcs_*.deb
fi

dpkg-query -l | grep pnetlab-dynamips | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-dynamips_*.deb
fi

dpkg-query -l | grep pnetlab-wireshark | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-wireshark_*.deb
fi

dpkg-query -l | grep pnetlab-qemu | grep 6.0.0-30 -q
if [ $? -ne 0 ]; then
    dpkg -i "$PACKAGES_DIR"/pnetlab-qemu_*.deb
fi
...
dpkg -i "$PACKAGES_DIR"/pnetlab_6*.deb
